...
// Copied from https://github.com/random-access-storage/random-access-file/blob/819f33c03c7db16ba1ad375114e27c845dece378/test.js

tape("write and read", async function(t) {
  raf(gen()).then(file => {
...
tape("read empty", function(t) {
  raf(gen(), { writable: true }).then(file => {
...
tape("read range > file", async function(t) {
  var file = raf(gen()).then(file => {
    file.read(0, 5, function(err, buf) {
...
tape("re-open and truncate", function(t) {
  ...
  raf(name).then(file => {
    file.write(10, Buffer.from("hello"), function(err) {
      t.error(err, "no error")
      raf(name, { truncate: true }).then(file2 => {
        file2.read(10, 5, function(err, buf) {
...
tape("truncate with size", function(t) {
  raf(gen(), { size: 100, writable: true }).then(file => {
...
tape("bad open", function(t) {
  raf(tmp, { writable: true }).then(file => {
...
tape("bad truncate", function(t) {
  raf(gen(), { writable: true, size: -1, truncate: true }).then(file => {
    file.open(function(err) {
      t.ok(err)
      file.destroy(() => t.end())
    })
...
tape("mkdir path", function(t) {
  var name = `${tmp}${++i}-folder/test.txt`
  raf(name).then(file => {
    file.write(0, Buffer.from("hello"), function(err) {
      t.error(err, "no error")
      file.read(0, 5, function(err, buf) {
...
let i = 0
const tmp = `/temp/random-access-file-${process.pid}-${Date.now()}`

const gen = () => `${tmp}-${++i}.txt`

const raf = async (path, options) => {
  const volume = await await RandomAccess.mount()
